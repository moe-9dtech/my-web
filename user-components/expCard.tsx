"use client";
import React from "react";
import { motion } from "framer-motion";
import Image from "next/image";
import Logo9D from "../public/assets/9d logo.jpeg";
import LogoUpWork from "../public/assets/upwork logo.png";
import SelfEmployee from "../public/assets/self employee.jpg";
import { SVGIcon } from "@/app/components/svgIcon";
import { SVGs } from "@/typings";
type Props = {
  company: string;
  title: string;
  at: string;
  start: string;
  end: string;
  techStack: string[];
  summary: string[];
};

export default function ExpCard({
  company,
  title,
  at,
  start,
  end,
  techStack,
  summary,
}: Props) {
  const svgs: SVGs = {
    JavaScript: `<svg className="w-10 h-10 rounded-full" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M1.4082 1.40796H126.592V126.593H1.4082V1.40796Z" fill="#F0DB4F"/>
<path d="M116.347 96.7361C115.43 91.0251 111.706 86.2281 100.675 81.7551C96.8431 79.9941 92.5711 78.7331 91.2981 75.8291C90.8461 74.1391 90.7861 73.1871 91.0721 72.1641C91.8931 68.8441 95.8561 67.8091 98.9971 68.7611C101.02 69.4391 102.935 70.9981 104.09 73.4851C109.492 69.9871 109.481 70.0101 113.253 67.6061C111.872 65.4651 111.135 64.4771 110.231 63.5611C106.982 59.9321 102.555 58.0631 95.4751 58.2061L91.7871 58.6831C88.2531 59.5761 84.8851 61.4311 82.9101 63.9181C76.9841 70.6421 78.6741 82.4101 85.8851 87.2531C92.9891 92.5851 103.425 93.7981 104.758 98.7841C106.055 104.888 100.272 106.864 94.5241 106.162C90.2881 105.281 87.9321 103.128 85.3851 99.2131C80.6971 101.926 80.6971 101.926 75.8771 104.698C77.0201 107.197 78.2211 108.328 80.1371 110.493C89.2051 119.691 111.897 119.239 115.967 105.317C116.132 104.839 117.228 101.651 116.347 96.7361ZM69.4621 58.9431H57.7531L57.7051 89.2151C57.7051 95.6531 58.0381 101.555 56.9911 103.364C55.2781 106.922 50.8391 106.481 48.8161 105.791C46.7571 104.779 45.7101 103.34 44.4971 101.306C44.1641 100.722 43.9141 100.27 43.8301 100.235L34.3101 106.065C35.8931 109.314 38.2251 112.134 41.2121 113.966C45.6741 116.644 51.6711 117.465 57.9431 116.025C62.0251 114.836 65.5471 112.373 67.3911 108.624C70.0571 103.709 69.4851 97.7601 69.4611 91.1801C69.5211 80.4451 69.4621 69.7121 69.4621 58.9431Z" fill="#323330"/>
</svg>`,
    React: `<svg
              className="w-10 h-10 rounded-full"
              fill="#5ED3F3"
              viewBox="0 0 512 512"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>React.js</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0" />
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              <g id="SVGRepo_iconCarrier">
                <path d="M410.66,180.72h0q-7.67-2.62-15.45-4.88,1.29-5.25,2.38-10.56c11.7-56.9,4.05-102.74-22.06-117.83-25-14.48-66,.61-107.36,36.69q-6.1,5.34-11.95,11-3.9-3.76-8-7.36c-43.35-38.58-86.8-54.83-112.88-39.69-25,14.51-32.43,57.6-21.9,111.53q1.58,8,3.55,15.93c-6.15,1.75-12.09,3.62-17.77,5.6C48.46,198.9,16,226.73,16,255.59c0,29.82,34.84,59.72,87.77,77.85q6.44,2.19,13,4.07Q114.64,346,113,354.68c-10,53-2.2,95.07,22.75,109.49,25.77,14.89,69-.41,111.14-37.31q5-4.38,10-9.25,6.32,6.11,13,11.86c40.8,35.18,81.09,49.39,106,34.93,25.75-14.94,34.12-60.14,23.25-115.13q-1.25-6.3-2.88-12.86,4.56-1.35,8.93-2.79c55-18.27,90.83-47.81,90.83-78C496,226.62,462.5,198.61,410.66,180.72Zm-129-81.08c35.43-30.91,68.55-43.11,83.65-34.39h0c16.07,9.29,22.32,46.75,12.22,95.88q-1,4.8-2.16,9.57a487.83,487.83,0,0,0-64.18-10.16,481.27,481.27,0,0,0-40.57-50.75Q276,104.57,281.64,99.64ZM157.73,280.25q6.51,12.6,13.61,24.89,7.23,12.54,15.07,24.71a435.28,435.28,0,0,1-44.24-7.13C146.41,309,151.63,294.75,157.73,280.25Zm0-48.33c-6-14.19-11.08-28.15-15.25-41.63,13.7-3.07,28.3-5.58,43.52-7.48q-7.65,11.94-14.72,24.23T157.7,231.92Zm10.9,24.17q9.48-19.77,20.42-38.78h0q10.93-19,23.27-37.13c14.28-1.08,28.92-1.65,43.71-1.65s29.52.57,43.79,1.66q12.21,18.09,23.13,37t20.69,38.6Q334,275.63,323,294.73h0q-10.91,19-23,37.24c-14.25,1-29,1.55-44,1.55s-29.47-.47-43.46-1.38q-12.43-18.19-23.46-37.29T168.6,256.09ZM340.75,305q7.25-12.58,13.92-25.49h0a440.41,440.41,0,0,1,16.12,42.32A434.44,434.44,0,0,1,326,329.48Q333.62,317.39,340.75,305Zm13.72-73.07q-6.64-12.65-13.81-25h0q-7-12.18-14.59-24.06c15.31,1.94,30,4.52,43.77,7.67A439.89,439.89,0,0,1,354.47,231.93ZM256.23,124.48h0a439.75,439.75,0,0,1,28.25,34.18q-28.35-1.35-56.74,0C237.07,146.32,246.62,134.87,256.23,124.48ZM145.66,65.86c16.06-9.32,51.57,4,89,37.27,2.39,2.13,4.8,4.36,7.2,6.67A491.37,491.37,0,0,0,201,160.51a499.12,499.12,0,0,0-64.06,10q-1.83-7.36-3.3-14.82h0C124.59,109.46,130.58,74.61,145.66,65.86ZM122.25,317.71q-6-1.71-11.85-3.71c-23.4-8-42.73-18.44-56-29.81C42.52,274,36.5,263.83,36.5,255.59c0-17.51,26.06-39.85,69.52-55q8.19-2.85,16.52-5.21a493.54,493.54,0,0,0,23.4,60.75A502.46,502.46,0,0,0,122.25,317.71Zm111.13,93.67c-18.63,16.32-37.29,27.89-53.74,33.72h0c-14.78,5.23-26.55,5.38-33.66,1.27-15.14-8.75-21.44-42.54-12.85-87.86q1.53-8,3.5-16a480.85,480.85,0,0,0,64.69,9.39,501.2,501.2,0,0,0,41.2,51C239.54,405.83,236.49,408.65,233.38,411.38Zm23.42-23.22c-9.72-10.51-19.42-22.14-28.88-34.64q13.79.54,28.08.54c9.78,0,19.46-.21,29-.64A439.33,439.33,0,0,1,256.8,388.16Zm124.52,28.59c-2.86,15.44-8.61,25.74-15.72,29.86-15.13,8.78-47.48-2.63-82.36-32.72-4-3.44-8-7.13-12.07-11a484.54,484.54,0,0,0,40.23-51.2,477.84,477.84,0,0,0,65-10.05q1.47,5.94,2.6,11.64h0C383.81,377.58,384.5,399.56,381.32,416.75Zm17.4-102.64h0c-2.62.87-5.32,1.71-8.06,2.53a483.26,483.26,0,0,0-24.31-60.94,481.52,481.52,0,0,0,23.36-60.06c4.91,1.43,9.68,2.93,14.27,4.52,44.42,15.32,71.52,38,71.52,55.43C475.5,274.19,446.23,298.33,398.72,314.11Z" />
                <path d="M256,298.55a43,43,0,1,0-42.86-43A42.91,42.91,0,0,0,256,298.55Z" />
              </g>
            </svg>`,
    Next: `<svg  className="w-10 h-10" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M23.749 30.005C23.63 30.068 23.64 30.088 23.754 30.03C23.7891 30.0157 23.8213 29.995 23.849 29.969C23.849 29.948 23.849 29.948 23.749 30.005ZM23.989 29.875C23.932 29.922 23.932 29.922 24 29.891C24.0251 29.8792 24.0481 29.8634 24.068 29.844C24.068 29.817 24.052 29.823 23.989 29.875ZM24.145 29.781C24.088 29.828 24.088 29.828 24.156 29.797C24.1812 29.785 24.2042 29.7688 24.224 29.749C24.224 29.724 24.208 29.729 24.145 29.781ZM24.303 29.688C24.246 29.735 24.246 29.735 24.312 29.703C24.349 29.683 24.38 29.662 24.38 29.656C24.38 29.631 24.364 29.636 24.303 29.688ZM24.516 29.547C24.407 29.62 24.369 29.667 24.469 29.615C24.536 29.574 24.65 29.484 24.63 29.484C24.587 29.5 24.551 29.527 24.515 29.547H24.516ZM14.953 0.0110121C14.88 0.0160121 14.661 0.0360121 14.469 0.0520121C9.921 0.464012 5.666 2.91201 2.969 6.68301C1.47856 8.75126 0.509758 11.1489 0.145 13.672C0.016 14.552 0 14.812 0 16.005C0 17.197 0.016 17.453 0.145 18.333C1.016 24.344 5.292 29.39 11.088 31.26C12.131 31.593 13.224 31.823 14.469 31.964C14.953 32.016 17.046 32.016 17.53 31.964C19.682 31.724 21.499 31.193 23.297 30.276C23.573 30.136 23.625 30.099 23.588 30.068C22.7109 28.9086 21.8412 27.7436 20.979 26.573L18.422 23.12L15.219 18.375C14.1536 16.7863 13.0772 15.2049 11.99 13.631C11.979 13.631 11.965 15.74 11.959 18.312C11.948 22.817 11.948 23 11.891 23.104C11.8363 23.2296 11.7384 23.3315 11.615 23.391C11.516 23.438 11.427 23.448 10.954 23.448H10.413L10.272 23.36C10.1837 23.3034 10.1119 23.2243 10.064 23.131L9.996 22.99L10.001 16.719L10.012 10.448L10.111 10.323C10.1742 10.2462 10.2521 10.1826 10.34 10.136C10.471 10.073 10.523 10.063 11.064 10.063C11.699 10.063 11.804 10.088 11.971 10.271C13.2685 12.2009 14.5549 14.1383 15.83 16.083C17.909 19.235 20.747 23.536 22.142 25.646L24.679 29.485L24.804 29.402C26.0232 28.5918 27.1281 27.6214 28.089 26.517C30.1045 24.2101 31.4124 21.3711 31.856 18.34C31.985 17.46 32.001 17.199 32.001 16.007C32.001 14.814 31.985 14.559 31.856 13.679C30.985 7.66801 26.709 2.62201 20.913 0.751012C19.8298 0.407933 18.7149 0.17443 17.585 0.0540121C17.282 0.0230121 15.214 -0.0139879 14.954 0.0130121L14.953 0.0110121ZM21.5 9.68801C21.5744 9.72455 21.6408 9.77556 21.6952 9.83806C21.7496 9.90057 21.791 9.9733 21.817 10.052C21.844 10.136 21.849 11.875 21.844 15.792L21.833 21.416L20.844 19.896L19.849 18.375V14.292C19.849 11.645 19.86 10.161 19.874 10.088C19.923 9.92063 20.0352 9.77893 20.187 9.69301C20.311 9.63001 20.359 9.62501 20.854 9.62501C21.317 9.62501 21.396 9.63001 21.5 9.68801Z" fill="black"/>
</svg>`,
    BootStrap: `<svg
              className="w-10 h-10 rounded-full"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              stroke=""
            >
              <title>Bootstrap</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M4.98488 2C3.61546 2 2.60217 3.19858 2.64753 4.49844C2.69105 5.74725 2.63451 7.36461 2.22732 8.68359C1.81892 10.0064 1.1282 10.8444 0 10.952V12.1666C1.1282 12.2742 1.81892 13.1122 2.22732 14.4351C2.63451 15.754 2.69105 17.3714 2.64753 18.6202C2.60217 19.9199 3.61546 21.1186 4.98508 21.1186H19.0169C20.3864 21.1186 21.3995 19.9201 21.3541 18.6202C21.3106 17.3714 21.3671 15.754 21.7743 14.4351C22.1829 13.1122 22.8718 12.2742 24 12.1666V10.952C22.8718 10.8444 22.1829 10.0064 21.7743 8.68359C21.3671 7.36481 21.3106 5.74725 21.3541 4.49844C21.3995 3.19878 20.3864 2 19.0169 2H4.98468H4.98488ZM16.2712 13.7687C16.2712 15.5586 14.9361 16.6441 12.7206 16.6441H8.94915C8.84127 16.6441 8.7378 16.6012 8.66152 16.5249C8.58523 16.4486 8.54237 16.3452 8.54237 16.2373V6.88136C8.54237 6.77347 8.58523 6.67001 8.66152 6.59372C8.7378 6.51743 8.84127 6.47458 8.94915 6.47458H12.6991C14.5464 6.47458 15.7588 7.47525 15.7588 9.01166C15.7588 10.09 14.9433 11.0555 13.9041 11.2245V11.2809C15.3187 11.4361 16.2712 12.4156 16.2712 13.7687ZM12.3094 7.76407H10.1589V10.8015H11.9701C13.3702 10.8015 14.1423 10.2377 14.1423 9.2299C14.1423 8.28556 13.4784 7.76407 12.3094 7.76407ZM10.1589 12.0068V15.3542H12.3885C13.8462 15.3542 14.6184 14.7692 14.6184 13.6699C14.6184 12.5704 13.8246 12.0066 12.2947 12.0066H10.1589V12.0068Z"
                  fill="#712cf9"
                ></path>
              </g>
            </svg>`,
    HTML: `<svg className="w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
      <path fill="#e44f26" d="M5.902 27.201L3.655 2h24.69l-2.25 25.197L15.985 30z"/><path fill="#f1662a" d="m16 27.858l8.17-2.265l1.922-21.532H16z"/>
      <path fill="#ebebeb" d="M16 13.407h-4.09l-.282-3.165H16V7.151H8.25l.074.83l.759 8.517H16zm0 8.027l-.014.004l-3.442-.929l-.22-2.465H9.221l.433 4.852l6.332 1.758l.014-.004z"/><path fill="#fff" d="M15.989 13.407v3.091h3.806l-.358 4.009l-3.448.93v3.216l6.337-1.757l.046-.522l.726-8.137l.076-.83h-.833zm0-6.256v3.091h7.466l.062-.694l.141-1.567l.074-.83z"/>
    </svg>`,
    CSS: `<svg
              className="w-10 h-10 rounded-full"
              xmlns="http://www.w3.org/2000/svg"
              aria-label="CSS3"
              role="img"
              viewBox="0 0 512 512"
              fill="#000000"
            >
              <title>CSS3</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <path fill="#264de4" d="M72 460L30 0h451l-41 460-184 52"></path>
                <path fill="#2965f1" d="M256 37V472l149-41 35-394"></path>
                <path
                  fill="#ebebeb"
                  d="m114 94h142v56H119m5 58h132v57H129m3 28h56l4 45 64 17v59L139 382"
                ></path>
                <path
                  fill="#ffffff"
                  d="m256 208v57h69l-7 73-62 17v59l115-32 26-288H256v56h80l-5.5 58Z"
                ></path>
              </g>
            </svg>`,
    TypeScript: `<svg
              className="w-10 h-10 rounded-lg"
              xmlns="http://www.w3.org/2000/svg"
              aria-label="TypeScript"
              role="img"
              viewBox="0 0 512 512"
              fill="#000000"
            >
              <title>Typescript</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <rect width="512" height="512" rx="15%" fill="#3178c6"></rect>
                <path
                  fill="#ffffff"
                  d="m233 284h64v-41H118v41h64v183h51zm84 173c8.1 4.2 18 7.3 29 9.4s23 3.1 35 3.1c12 0 23-1.1 34-3.4c11-2.3 20-6.1 28-11c8.1-5.3 15-12 19-21s7.1-19 7.1-32c0-9.1-1.4-17-4.1-24s-6.6-13-12-18c-5.1-5.3-11-10-18-14s-15-8.2-24-12c-6.6-2.7-12-5.3-18-7.9c-5.2-2.6-9.7-5.2-13-7.8c-3.7-2.7-6.5-5.5-8.5-8.4c-2-3-3-6.3-3-10c0-3.4.89-6.5 2.7-9.3s4.3-5.1 7.5-7.1c3.2-2 7.2-3.5 12-4.6c4.7-1.1 9.9-1.6 16-1.6c4.2 0 8.6.31 13 .94c4.6.63 9.3 1.6 14 2.9c4.7 1.3 9.3 2.9 14 4.9c4.4 2 8.5 4.3 12 6.9v-47c-7.6-2.9-16-5.1-25-6.5s-19-2.1-31-2.1c-12 0-23 1.3-34 3.8s-20 6.5-28 12c-8.1 5.4-14 12-19 21c-4.7 8.4-7 18-7 30c0 15 4.3 28 13 38c8.6 11 22 19 39 27c6.9 2.8 13 5.6 19 8.3s11 5.5 15 8.4c4.3 2.9 7.7 6.1 10 9.5c2.5 3.4 3.8 7.4 3.8 12c0 3.2-.78 6.2-2.3 9s-3.9 5.2-7.1 7.2s-7.1 3.6-12 4.8c-4.7 1.1-10 1.7-17 1.7c-11 0-22-1.9-32-5.7c-11-3.8-21-9.5-28.1-15.44z"
                ></path>
              </g>
            </svg>`,
    Tailwind: `<svg className="w-10 h-10" viewBox="0 0 256 154" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_115_66)">
<path d="M128 0C93.867 0 72.533 17.067 64 51.2C76.8 34.133 91.733 27.733 108.8 32C118.537 34.434 125.497 41.499 133.201 49.318C145.751 62.057 160.275 76.8 192 76.8C226.133 76.8 247.467 59.733 256 25.6C243.2 42.667 228.267 49.067 211.2 44.8C201.463 42.366 194.503 35.301 186.799 27.482C174.249 14.743 159.725 0 128 0ZM64 76.8C29.867 76.8 8.533 93.867 0 128C12.8 110.933 27.733 104.533 44.8 108.8C54.537 111.234 61.497 118.299 69.201 126.118C81.751 138.857 96.275 153.6 128 153.6C162.133 153.6 183.467 136.533 192 102.4C179.2 119.467 164.267 125.867 147.2 121.6C137.463 119.166 130.503 112.101 122.799 104.282C110.249 91.543 95.725 76.8 64 76.8Z" fill="url(#paint0_linear_115_66)"/>
</g>
<defs>
<linearGradient id="paint0_linear_115_66" x1="-711.168" y1="4915.2" x2="21398.6" y2="17663.3" gradientUnits="userSpaceOnUse">
<stop stop-color="#2298BD"/>
<stop offset="1" stop-color="#0ED7B5"/>
</linearGradient>
<clipPath id="clip0_115_66">
<rect width="256" height="154" fill="white"/>
</clipPath>
</defs>
</svg>
`,
    wordPress: `<svg className="w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256">
    <g fill="none"><rect width="256" height="256" fill="#0073aa" rx="60"/><path fill="#fff" d="M42.214 127.994c0 33.955 19.733 63.299 48.347 77.205L49.641 93.083a85.46 85.46 0 0 0-7.427 34.911m143.694-4.329c0-10.601-3.808-17.943-7.074-23.658c-4.349-7.066-8.425-13.05-8.425-20.116c0-7.886 5.981-15.226 14.405-15.226c.381 0 .741.047 1.112.068c-15.262-13.982-35.595-22.519-57.929-22.519c-29.968 0-56.335 15.376-71.673 38.666c2.012.06 3.909.103 5.52.103c8.973 0 22.862-1.09 22.862-1.09c4.624-.272 5.17 6.52.55 7.067c0 0-4.647.547-9.818.818l31.238 92.918l18.773-56.303l-13.365-36.618c-4.619-.271-8.995-.818-8.995-.818c-4.623-.271-4.081-7.339.542-7.066c0 0 14.166 1.088 22.595 1.088c8.972 0 22.862-1.088 22.862-1.088c4.628-.273 5.171 6.519.55 7.066c0 0-4.657.547-9.818.818l31.001 92.214l8.556-28.592c3.709-11.866 6.531-20.388 6.531-27.732"/><path fill="#fff" d="m129.503 135.498l-25.738 74.79a85.779 85.779 0 0 0 24.233 3.495a85.682 85.682 0 0 0 28.486-4.863a7.476 7.476 0 0 1-.61-1.182zm73.766-48.66a65.8 65.8 0 0 1 .578 8.82c0 8.705-1.626 18.491-6.523 30.727l-26.203 75.759c25.503-14.872 42.657-42.501 42.657-74.148c.001-14.915-3.808-28.94-10.509-41.158"/><path fill="#fff" d="M127.998 28C72.86 28 28 72.857 28 127.994c0 55.144 44.86 99.999 99.998 99.999c55.135 0 100.002-44.855 100.002-99.999C227.998 72.857 183.133 28 127.998 28m0 195.41c-52.61 0-95.413-42.804-95.413-95.416c0-52.609 42.802-95.409 95.413-95.409c52.607 0 95.407 42.8 95.407 95.409c0 52.612-42.8 95.416-95.407 95.416"/>
    </g>
    </svg>`,
    PHP: `<svg
              className="w-10 h-10 rounded-full"
              viewBox="0 0 32 32"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>PHP</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <circle cx="16" cy="16" r="14" fill="#8892BF"></circle>{" "}
                <path
                  d="M14.4392 10H16.1192L15.6444 12.5242H17.154C17.9819 12.5419 18.5986 12.7269 19.0045 13.0793C19.4184 13.4316 19.5402 14.1014 19.3698 15.0881L18.5541 19.4889H16.8497L17.6288 15.2863C17.7099 14.8457 17.6856 14.533 17.5558 14.348C17.426 14.163 17.146 14.0705 16.7158 14.0705L15.3644 14.0573L14.3661 19.4889H12.6861L14.4392 10Z"
                  fill="white"
                ></path>{" "}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M6.74092 12.5243H10.0036C10.9612 12.533 11.6552 12.8327 12.0854 13.4229C12.5156 14.0132 12.6576 14.8193 12.5115 15.8414C12.4548 16.3085 12.3289 16.7665 12.1341 17.2159C11.9474 17.6652 11.6878 18.0704 11.355 18.4317C10.9491 18.8898 10.5149 19.1805 10.0523 19.304C9.58969 19.4274 9.11076 19.489 8.61575 19.489H7.15484L6.69222 22H5L6.74092 12.5243ZM7.43485 17.9956L8.16287 14.0441H8.40879C8.49815 14.0441 8.5914 14.0396 8.6888 14.0309C9.33817 14.0221 9.87774 14.0882 10.308 14.2291C10.7462 14.37 10.8923 14.9031 10.7462 15.8282C10.5678 16.9296 10.2186 17.5727 9.69926 17.7577C9.1799 17.934 8.53053 18.0176 7.75138 18.0088H7.58094C7.53224 18.0088 7.48355 18.0043 7.43485 17.9956Z"
                  fill="white"
                ></path>{" "}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M24.4365 12.5243H21.1738L19.4329 22H21.1251L21.5878 19.489H23.0487C23.5437 19.489 24.0226 19.4274 24.4852 19.304C24.9479 19.1805 25.382 18.8898 25.7879 18.4317C26.1207 18.0704 26.3803 17.6652 26.567 17.2159C26.7618 16.7665 26.8877 16.3085 26.9444 15.8414C27.0905 14.8193 26.9486 14.0132 26.5183 13.4229C26.0881 12.8327 25.3942 12.533 24.4365 12.5243ZM22.5958 14.0441L21.8678 17.9956C21.9165 18.0043 21.9652 18.0088 22.0139 18.0088H22.1843C22.9635 18.0176 23.6128 17.934 24.1322 17.7577C24.6515 17.5727 25.0007 16.9296 25.1792 15.8282C25.3253 14.9031 25.1792 14.37 24.7409 14.2291C24.3107 14.0882 23.7711 14.0221 23.1217 14.0309C23.0243 14.0396 22.9311 14.0441 22.8417 14.0441H22.5958Z"
                  fill="white"
                ></path>{" "}
              </g>
            </svg>`,
    Node: `<svg className="w-10 h-10 rounded-full" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M16 30.0001C15.6222 30.0002 15.2511 29.9009 14.924 29.7121L11.5 27.6851C10.989 27.3991 11.238 27.2981 11.407 27.2391C11.9504 27.0712 12.4708 26.836 12.956 26.5391C12.9966 26.5203 13.0413 26.5122 13.0859 26.5155C13.1306 26.5188 13.1736 26.5335 13.211 26.5581L15.842 28.1211C15.891 28.147 15.9455 28.1606 16.001 28.1606C16.0564 28.1606 16.111 28.147 16.16 28.1211L26.42 22.1991C26.468 22.1703 26.5077 22.1296 26.5352 22.0809C26.5627 22.0321 26.5771 21.9771 26.577 21.9211V10.0751C26.577 10.0182 26.5624 9.96234 26.5345 9.91277C26.5067 9.8632 26.4665 9.82164 26.418 9.7921L16.158 3.8751C16.1096 3.84785 16.055 3.83354 15.9995 3.83354C15.9439 3.83354 15.8893 3.84785 15.841 3.8751L5.58697 9.7941C5.53806 9.82303 5.49744 9.86409 5.46906 9.91332C5.44068 9.96255 5.42549 10.0183 5.42497 10.0751V21.9161C5.42511 21.9719 5.44009 22.0267 5.46838 22.0749C5.49667 22.123 5.53725 22.1628 5.58597 22.1901L8.39997 23.8141C9.92497 24.5761 10.859 23.6781 10.859 22.7761V11.0851C10.859 11.0055 10.8906 10.9292 10.9468 10.873C11.0031 10.8167 11.0794 10.7851 11.159 10.7851H12.459C12.5385 10.7851 12.6148 10.8167 12.6711 10.873C12.7274 10.9292 12.759 11.0055 12.759 11.0851V22.7771C12.759 24.8121 11.651 25.9771 9.72097 25.9771C8.88835 25.9916 8.06878 25.7689 7.35797 25.3351L4.66097 23.7881C4.33356 23.5974 4.06188 23.3242 3.87308 22.9957C3.68427 22.6672 3.58492 22.295 3.58497 21.9161V10.0751C3.58399 9.69563 3.68292 9.32259 3.87179 8.99346C4.06067 8.66433 4.33284 8.39071 4.66097 8.2001L14.922 2.2761C15.2525 2.09527 15.6232 2.00049 16 2.00049C16.3767 2.00049 16.7474 2.09527 17.078 2.2761L27.338 8.2001C27.6653 8.39039 27.9371 8.66322 28.1261 8.99135C28.315 9.31948 28.4147 9.69144 28.415 10.0701V21.9161C28.4144 22.295 28.3147 22.6671 28.1258 22.9955C27.9368 23.3239 27.6652 23.5972 27.338 23.7881L17.078 29.7121C16.7502 29.9012 16.3784 30.0005 16 30.0001Z" fill="#83CD29"/>
<path d="M14.0542 17.9531C14.0542 17.8736 14.0858 17.7972 14.1421 17.741C14.1983 17.6847 14.2746 17.6531 14.3542 17.6531H15.6812C15.7521 17.6533 15.8206 17.6787 15.8746 17.7246C15.9286 17.7706 15.9646 17.8342 15.9762 17.9041C16.1762 19.2551 16.7762 19.9361 19.4892 19.9361C21.6502 19.9361 22.5712 19.4471 22.5712 18.3001C22.5712 17.6391 22.3102 17.1481 18.9512 16.8191C16.1432 16.5411 14.4072 15.9191 14.4072 13.6751C14.4072 11.6051 16.1522 10.3701 19.0772 10.3701C22.3642 10.3701 23.9912 11.5111 24.1972 13.9591C24.2003 14 24.1951 14.0411 24.1817 14.0798C24.1684 14.1186 24.1472 14.1542 24.1196 14.1845C24.092 14.2147 24.0584 14.239 24.021 14.2558C23.9836 14.2726 23.9432 14.2816 23.9022 14.2821H22.5662C22.4992 14.2812 22.4344 14.2578 22.3822 14.2158C22.33 14.1737 22.2934 14.1154 22.2782 14.0501C21.9592 12.6291 21.1782 12.1751 19.0782 12.1751C16.7182 12.1751 16.4442 12.9971 16.4442 13.6131C16.4442 14.3591 16.7682 14.5771 19.9542 14.9981C23.1072 15.4151 24.6052 16.0051 24.6052 18.2211C24.6052 20.4571 22.7412 21.7371 19.4902 21.7371C14.9952 21.7431 14.0542 19.6821 14.0542 17.9531Z" fill="#83CD29"/>
</svg>`,
  };

  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 1.5 }}
    >
      <article className="flex flex-col rounded-lg items-center space-y-7 flex-shrink-0 w-full md:w-[600px] lg:w-[900px] snap-center bg-[#292929] p-10 opacity-40 hover:opacity-100 cursor-pointer transition-opacity duration-300 overflow-hidden">
        <motion.div
          initial={{
            y: -100,
            opacity: 0,
          }}
          whileInView={{
            y: 0,
            opacity: 1,
          }}
          transition={{ duration: 1.5 }}
        >
             {company === 'Logo9D' && <Image src={Logo9D} alt="9D Technologies" className="rounded-full w-32 h-32 xl:w-[200px] xl:h-[200px] object-cover object-center shadow-md" />}
    {company === 'LogoUpWork' && <Image src={LogoUpWork} alt="UpWork Logo" className="rounded-full w-32 h-32 xl:w-[200px] xl:h-[200px] object-cover object-center shadow-md" />}
    {company === 'SelfEmployee' && <Image src={SelfEmployee} alt="Self Employee Logo" className="rounded-full w-32 h-32 xl:w-[200px] xl:h-[200px] object-cover object-center shadow-md" />}

          {/* <Image
            src={company}
            width={1}
            height={1}
            alt="9D Technologies"
            className="rounded-full w-32 h-32 xl:w-[200px] xl:h-[200px] object-cover object-center"
          /> */}
        </motion.div>

        <div className="">
          <h4 className="text-4xl font-light">{title}</h4>
          <p className="font-bold text-xl mt-1">{at}</p>
          <div className="flex space-x-2 my-2">
            {techStack &&
              techStack.map((tech, id) => (
                <SVGIcon key={id} svg={svgs[tech]} />
              ))}

            {/* js */}

            {/* <svg
              className="w-10 h-10 rounded-full"
              viewBox="0 0 32 32"
              xmlns="http://www.w3.org/2000/svg"
              fill="#000000"
            >
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <title>JavaScript</title>
                <rect
                  x="2"
                  y="2"
                  width="28"
                  height="28"
                  style={{ fill: "#f5de19" }}
                ></rect>
                <path d="M20.809,23.875a2.866,2.866,0,0,0,2.6,1.6c1.09,0,1.787-.545,1.787-1.3,0-.9-.716-1.222-1.916-1.747l-.658-.282c-1.9-.809-3.16-1.822-3.16-3.964,0-1.973,1.5-3.476,3.853-3.476a3.889,3.889,0,0,1,3.742,2.107L25,18.128A1.789,1.789,0,0,0,23.311,17a1.145,1.145,0,0,0-1.259,1.128c0,.789.489,1.109,1.618,1.6l.658.282c2.236.959,3.5,1.936,3.5,4.133,0,2.369-1.861,3.667-4.36,3.667a5.055,5.055,0,0,1-4.795-2.691Zm-9.295.228c.413.733.789,1.353,1.693,1.353.864,0,1.41-.338,1.41-1.653V14.856h2.631v8.982c0,2.724-1.6,3.964-3.929,3.964a4.085,4.085,0,0,1-3.947-2.4Z"></path>
              </g>
            </svg> */}

            {/* React.js */}
            {/* <svg
              className="w-10 h-10 rounded-full"
              fill="#5ED3F3"
              viewBox="0 0 512 512"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>React.js</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0" />
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              <g id="SVGRepo_iconCarrier">
                <path d="M410.66,180.72h0q-7.67-2.62-15.45-4.88,1.29-5.25,2.38-10.56c11.7-56.9,4.05-102.74-22.06-117.83-25-14.48-66,.61-107.36,36.69q-6.1,5.34-11.95,11-3.9-3.76-8-7.36c-43.35-38.58-86.8-54.83-112.88-39.69-25,14.51-32.43,57.6-21.9,111.53q1.58,8,3.55,15.93c-6.15,1.75-12.09,3.62-17.77,5.6C48.46,198.9,16,226.73,16,255.59c0,29.82,34.84,59.72,87.77,77.85q6.44,2.19,13,4.07Q114.64,346,113,354.68c-10,53-2.2,95.07,22.75,109.49,25.77,14.89,69-.41,111.14-37.31q5-4.38,10-9.25,6.32,6.11,13,11.86c40.8,35.18,81.09,49.39,106,34.93,25.75-14.94,34.12-60.14,23.25-115.13q-1.25-6.3-2.88-12.86,4.56-1.35,8.93-2.79c55-18.27,90.83-47.81,90.83-78C496,226.62,462.5,198.61,410.66,180.72Zm-129-81.08c35.43-30.91,68.55-43.11,83.65-34.39h0c16.07,9.29,22.32,46.75,12.22,95.88q-1,4.8-2.16,9.57a487.83,487.83,0,0,0-64.18-10.16,481.27,481.27,0,0,0-40.57-50.75Q276,104.57,281.64,99.64ZM157.73,280.25q6.51,12.6,13.61,24.89,7.23,12.54,15.07,24.71a435.28,435.28,0,0,1-44.24-7.13C146.41,309,151.63,294.75,157.73,280.25Zm0-48.33c-6-14.19-11.08-28.15-15.25-41.63,13.7-3.07,28.3-5.58,43.52-7.48q-7.65,11.94-14.72,24.23T157.7,231.92Zm10.9,24.17q9.48-19.77,20.42-38.78h0q10.93-19,23.27-37.13c14.28-1.08,28.92-1.65,43.71-1.65s29.52.57,43.79,1.66q12.21,18.09,23.13,37t20.69,38.6Q334,275.63,323,294.73h0q-10.91,19-23,37.24c-14.25,1-29,1.55-44,1.55s-29.47-.47-43.46-1.38q-12.43-18.19-23.46-37.29T168.6,256.09ZM340.75,305q7.25-12.58,13.92-25.49h0a440.41,440.41,0,0,1,16.12,42.32A434.44,434.44,0,0,1,326,329.48Q333.62,317.39,340.75,305Zm13.72-73.07q-6.64-12.65-13.81-25h0q-7-12.18-14.59-24.06c15.31,1.94,30,4.52,43.77,7.67A439.89,439.89,0,0,1,354.47,231.93ZM256.23,124.48h0a439.75,439.75,0,0,1,28.25,34.18q-28.35-1.35-56.74,0C237.07,146.32,246.62,134.87,256.23,124.48ZM145.66,65.86c16.06-9.32,51.57,4,89,37.27,2.39,2.13,4.8,4.36,7.2,6.67A491.37,491.37,0,0,0,201,160.51a499.12,499.12,0,0,0-64.06,10q-1.83-7.36-3.3-14.82h0C124.59,109.46,130.58,74.61,145.66,65.86ZM122.25,317.71q-6-1.71-11.85-3.71c-23.4-8-42.73-18.44-56-29.81C42.52,274,36.5,263.83,36.5,255.59c0-17.51,26.06-39.85,69.52-55q8.19-2.85,16.52-5.21a493.54,493.54,0,0,0,23.4,60.75A502.46,502.46,0,0,0,122.25,317.71Zm111.13,93.67c-18.63,16.32-37.29,27.89-53.74,33.72h0c-14.78,5.23-26.55,5.38-33.66,1.27-15.14-8.75-21.44-42.54-12.85-87.86q1.53-8,3.5-16a480.85,480.85,0,0,0,64.69,9.39,501.2,501.2,0,0,0,41.2,51C239.54,405.83,236.49,408.65,233.38,411.38Zm23.42-23.22c-9.72-10.51-19.42-22.14-28.88-34.64q13.79.54,28.08.54c9.78,0,19.46-.21,29-.64A439.33,439.33,0,0,1,256.8,388.16Zm124.52,28.59c-2.86,15.44-8.61,25.74-15.72,29.86-15.13,8.78-47.48-2.63-82.36-32.72-4-3.44-8-7.13-12.07-11a484.54,484.54,0,0,0,40.23-51.2,477.84,477.84,0,0,0,65-10.05q1.47,5.94,2.6,11.64h0C383.81,377.58,384.5,399.56,381.32,416.75Zm17.4-102.64h0c-2.62.87-5.32,1.71-8.06,2.53a483.26,483.26,0,0,0-24.31-60.94,481.52,481.52,0,0,0,23.36-60.06c4.91,1.43,9.68,2.93,14.27,4.52,44.42,15.32,71.52,38,71.52,55.43C475.5,274.19,446.23,298.33,398.72,314.11Z" />
                <path d="M256,298.55a43,43,0,1,0-42.86-43A42.91,42.91,0,0,0,256,298.55Z" />
              </g>
            </svg> */}

            {/* Next.js */}
            {/* <svg
              className="w-10 h-10 rounded-full align-middle"
              aria-label="Next.js logomark"
              height="80"
              role="img"
              viewBox="0 0 180 180"
              width="80"
            >
              <title>Next.js</title>
              <mask
                height="180"
                id=":R0:mask0_408_134"
                maskUnits="userSpaceOnUse"
                style={{ maskType: "alpha" }}
                width="180"
                x="0"
                y="0"
              >
                <circle cx="90" cy="90" fill="black" r="90"></circle>
              </mask>
              <g mask="url(#:R0:mask0_408_134)">
                <circle
                  cx="90"
                  cy="90"
                  data-circle="true"
                  fill="black"
                  r="90"
                ></circle>
                <path
                  d="M149.508 157.52L69.142 54H54V125.97H66.1136V69.3836L139.999 164.845C143.333 162.614 146.509 160.165 149.508 157.52Z"
                  fill="url(#:R0:paint0_linear_408_134)"
                ></path>
                <rect
                  fill="url(#:R0:paint1_linear_408_134)"
                  height="72"
                  width="12"
                  x="115"
                  y="54"
                ></rect>
              </g>
              <defs>
                <linearGradient
                  gradientUnits="userSpaceOnUse"
                  id=":R0:paint0_linear_408_134"
                  x1="109"
                  x2="144.5"
                  y1="116.5"
                  y2="160.5"
                >
                  <stop stopColor="white"></stop>
                  <stop offset="1" stopColor="white" stopOpacity="0"></stop>
                </linearGradient>
                <linearGradient
                  gradientUnits="userSpaceOnUse"
                  id=":R0:paint1_linear_408_134"
                  x1="121"
                  x2="120.799"
                  y1="54"
                  y2="106.875"
                >
                  <stop stopColor="white"></stop>
                  <stop offset="1" stopColor="white" stopOpacity="0"></stop>
                </linearGradient>
              </defs>
            </svg> */}

            {/* Bootstrap */}
            {/* <svg
              className="w-10 h-10 rounded-full"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              stroke=""
            >
              <title>Bootstrap</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M4.98488 2C3.61546 2 2.60217 3.19858 2.64753 4.49844C2.69105 5.74725 2.63451 7.36461 2.22732 8.68359C1.81892 10.0064 1.1282 10.8444 0 10.952V12.1666C1.1282 12.2742 1.81892 13.1122 2.22732 14.4351C2.63451 15.754 2.69105 17.3714 2.64753 18.6202C2.60217 19.9199 3.61546 21.1186 4.98508 21.1186H19.0169C20.3864 21.1186 21.3995 19.9201 21.3541 18.6202C21.3106 17.3714 21.3671 15.754 21.7743 14.4351C22.1829 13.1122 22.8718 12.2742 24 12.1666V10.952C22.8718 10.8444 22.1829 10.0064 21.7743 8.68359C21.3671 7.36481 21.3106 5.74725 21.3541 4.49844C21.3995 3.19878 20.3864 2 19.0169 2H4.98468H4.98488ZM16.2712 13.7687C16.2712 15.5586 14.9361 16.6441 12.7206 16.6441H8.94915C8.84127 16.6441 8.7378 16.6012 8.66152 16.5249C8.58523 16.4486 8.54237 16.3452 8.54237 16.2373V6.88136C8.54237 6.77347 8.58523 6.67001 8.66152 6.59372C8.7378 6.51743 8.84127 6.47458 8.94915 6.47458H12.6991C14.5464 6.47458 15.7588 7.47525 15.7588 9.01166C15.7588 10.09 14.9433 11.0555 13.9041 11.2245V11.2809C15.3187 11.4361 16.2712 12.4156 16.2712 13.7687ZM12.3094 7.76407H10.1589V10.8015H11.9701C13.3702 10.8015 14.1423 10.2377 14.1423 9.2299C14.1423 8.28556 13.4784 7.76407 12.3094 7.76407ZM10.1589 12.0068V15.3542H12.3885C13.8462 15.3542 14.6184 14.7692 14.6184 13.6699C14.6184 12.5704 13.8246 12.0066 12.2947 12.0066H10.1589V12.0068Z"
                  fill="#712cf9"
                ></path>
              </g>
            </svg> */}

            {/* HTML5 */}
            {/* <svg
              className="w-10 h-10 rounded-full"
              viewBox="0 0 32 32"
              xmlns="http://www.w3.org/2000/svg"
              fill="#000000"
            >
              <title>HTML5</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <title>file_type_html</title>
                <polygon
                  points="5.902 27.201 3.655 2 28.345 2 26.095 27.197 15.985 30 5.902 27.201"
                  style={{ fill: "#e44f26" }}
                ></polygon>
                <polygon
                  points="16 27.858 24.17 25.593 26.092 4.061 16 4.061 16 27.858"
                  style={{ fill: "#f1662a" }}
                ></polygon>
                <polygon
                  points="16 13.407 11.91 13.407 11.628 10.242 16 10.242 16 7.151 15.989 7.151 8.25 7.151 8.324 7.981 9.083 16.498 16 16.498 16 13.407"
                  style={{ fill: "#ebebeb" }}
                ></polygon>
                <polygon
                  points="16 21.434 15.986 21.438 12.544 20.509 12.324 18.044 10.651 18.044 9.221 18.044 9.654 22.896 15.986 24.654 16 24.65 16 21.434"
                  style={{ fill: "#ebebeb" }}
                ></polygon>
                <polygon
                  points="15.989 13.407 15.989 16.498 19.795 16.498 19.437 20.507 15.989 21.437 15.989 24.653 22.326 22.896 22.372 22.374 23.098 14.237 23.174 13.407 22.341 13.407 15.989 13.407"
                  style={{ fill: "#fff" }}
                ></polygon>
                <polygon
                  points="15.989 7.151 15.989 9.071 15.989 10.235 15.989 10.242 23.445 10.242 23.445 10.242 23.455 10.242 23.517 9.548 23.658 7.981 23.732 7.151 15.989 7.151"
                  style={{ fill: "#fff" }}
                ></polygon>
              </g>
            </svg> */}

            {/* css3 */}
            {/* <svg
              className="w-10 h-10 rounded-full"
              xmlns="http://www.w3.org/2000/svg"
              aria-label="CSS3"
              role="img"
              viewBox="0 0 512 512"
              fill="#000000"
            >
              <title>CSS3</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <path fill="#264de4" d="M72 460L30 0h451l-41 460-184 52"></path>
                <path fill="#2965f1" d="M256 37V472l149-41 35-394"></path>
                <path
                  fill="#ebebeb"
                  d="m114 94h142v56H119m5 58h132v57H129m3 28h56l4 45 64 17v59L139 382"
                ></path>
                <path
                  fill="#ffffff"
                  d="m256 208v57h69l-7 73-62 17v59l115-32 26-288H256v56h80l-5.5 58Z"
                ></path>
              </g>
            </svg> */}

            {/* Typescript */}
            {/* <svg
              className="w-10 h-10 rounded-lg"
              xmlns="http://www.w3.org/2000/svg"
              aria-label="TypeScript"
              role="img"
              viewBox="0 0 512 512"
              fill="#000000"
            >
              <title>Typescript</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                <rect width="512" height="512" rx="15%" fill="#3178c6"></rect>
                <path
                  fill="#ffffff"
                  d="m233 284h64v-41H118v41h64v183h51zm84 173c8.1 4.2 18 7.3 29 9.4s23 3.1 35 3.1c12 0 23-1.1 34-3.4c11-2.3 20-6.1 28-11c8.1-5.3 15-12 19-21s7.1-19 7.1-32c0-9.1-1.4-17-4.1-24s-6.6-13-12-18c-5.1-5.3-11-10-18-14s-15-8.2-24-12c-6.6-2.7-12-5.3-18-7.9c-5.2-2.6-9.7-5.2-13-7.8c-3.7-2.7-6.5-5.5-8.5-8.4c-2-3-3-6.3-3-10c0-3.4.89-6.5 2.7-9.3s4.3-5.1 7.5-7.1c3.2-2 7.2-3.5 12-4.6c4.7-1.1 9.9-1.6 16-1.6c4.2 0 8.6.31 13 .94c4.6.63 9.3 1.6 14 2.9c4.7 1.3 9.3 2.9 14 4.9c4.4 2 8.5 4.3 12 6.9v-47c-7.6-2.9-16-5.1-25-6.5s-19-2.1-31-2.1c-12 0-23 1.3-34 3.8s-20 6.5-28 12c-8.1 5.4-14 12-19 21c-4.7 8.4-7 18-7 30c0 15 4.3 28 13 38c8.6 11 22 19 39 27c6.9 2.8 13 5.6 19 8.3s11 5.5 15 8.4c4.3 2.9 7.7 6.1 10 9.5c2.5 3.4 3.8 7.4 3.8 12c0 3.2-.78 6.2-2.3 9s-3.9 5.2-7.1 7.2s-7.1 3.6-12 4.8c-4.7 1.1-10 1.7-17 1.7c-11 0-22-1.9-32-5.7c-11-3.8-21-9.5-28.1-15.44z"
                ></path>
              </g>
            </svg> */}

            {/* Tailwind */}
            {/* <svg
              className="w-10 h-10 rounded-full"
              viewBox="0 -51 256 256"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
              preserveAspectRatio="xMidYMid"
              fill="#000000"
            >
              <title>Tailwind CSS</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <defs>
                  {" "}
                  <linearGradient
                    x1="-2.77777778%"
                    y1="32%"
                    x2="100%"
                    y2="67.5555556%"
                    id="linearGradient-1"
                  >
                    {" "}
                    <stop stopColor="#2298BD" offset="0%">
                      {" "}
                    </stop>{" "}
                    <stop stopColor="#0ED7B5" offset="100%">
                      {" "}
                    </stop>{" "}
                  </linearGradient>{" "}
                </defs>{" "}
                <g>
                  {" "}
                  <path
                    d="M128,-1.0658141e-14 C93.8666667,-1.0658141e-14 72.5333333,17.0666667 64,51.2 C76.8,34.1333333 91.7333333,27.7333333 108.8,32 C118.537481,34.4343704 125.497363,41.4985481 133.201067,49.3184 C145.750756,62.0567704 160.275437,76.8 192,76.8 C226.133333,76.8 247.466667,59.7333333 256,25.6 C243.2,42.6666667 228.266667,49.0666667 211.2,44.8 C201.462519,42.3656296 194.502637,35.3014519 186.798933,27.4816 C174.249244,14.7432296 159.724563,-1.0658141e-14 128,-1.0658141e-14 Z M64,76.8 C29.8666667,76.8 8.53333333,93.8666667 0,128 C12.8,110.933333 27.7333333,104.533333 44.8,108.8 C54.5374815,111.23437 61.497363,118.298548 69.2010667,126.1184 C81.7507556,138.85677 96.275437,153.6 128,153.6 C162.133333,153.6 183.466667,136.533333 192,102.4 C179.2,119.466667 164.266667,125.866667 147.2,121.6 C137.462519,119.16563 130.502637,112.101452 122.798933,104.2816 C110.249244,91.5432296 95.724563,76.8 64,76.8 Z"
                    fill="url(#linearGradient-1)"
                  >
                    {" "}
                  </path>{" "}
                </g>{" "}
              </g>
            </svg> */}

            {/* Wordpress */}
            {/* <svg
              className="w-10 h-10 rounded-full"
              fill="#dedede"
              version="1.1"
              id="Layer_1"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
              width="64px"
              height="64px"
              viewBox="0 0 512 512"
              enableBackground="new 0 0 512 512"
              xmlSpace="preserve"
              stroke="#dedede"
            >
              <title>Wordpress</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <g>
                  {" "}
                  <path d="M256,0C114.609,0,0,114.609,0,256s114.609,256,256,256s256-114.609,256-256S397.391,0,256,0z M256,472 c-119.297,0-216-96.703-216-216S136.703,40,256,40s216,96.703,216,216S375.297,472,256,472z"></path>{" "}
                  <path d="M128,256c0,50.625,29.438,94.438,72.156,115.156l-61.078-167.25C131.969,219.828,128,237.438,128,256z"></path>{" "}
                  <path d="M258.266,267.203l-38.422,111.594c11.469,3.359,23.594,5.203,36.172,5.203c14.891,0,29.188-2.594,42.5-7.281 c-0.344-0.547-0.656-1.094-0.906-1.75L258.266,267.203z"></path>{" "}
                  <path d="M342.406,249.547c0-15.828-5.688-26.797-10.547-35.312c-6.484-10.547-12.562-19.453-12.562-30.016 c0-11.766,8.906-22.703,21.5-22.703c0.547,0,1.094,0.062,1.641,0.109C319.672,140.75,289.344,128,256.016,128 c-44.734,0-84.078,22.953-106.953,57.688c3,0.109,5.828,0.172,8.234,0.172c13.391,0,34.141-1.641,34.141-1.641 c6.875-0.391,7.688,9.719,0.781,10.562c0,0-6.906,0.797-14.656,1.219l46.625,138.625l28.016-84L232.25,196 c-6.891-0.422-13.422-1.219-13.422-1.219c-6.891-0.422-6.078-10.953,0.828-10.562c0,0,21.125,1.641,33.703,1.641 c13.375,0,34.125-1.641,34.125-1.641c6.891-0.391,7.703,9.719,0.797,10.562c0,0-6.906,0.797-14.641,1.219l46.266,137.562 l12.766-42.656C339.156,274.281,342.406,260.5,342.406,249.547z"></path>{" "}
                  <path d="M369.203,207.766c0,12.969-2.438,27.562-9.75,45.828l-39.078,113.031C358.406,344.438,384,303.219,384,256 c0-22.266-5.672-43.172-15.672-61.422C368.875,198.688,369.203,203.047,369.203,207.766z"></path>{" "}
                </g>{" "}
              </g>
            </svg> */}

            {/* PHP */}
            {/* <svg
              className="w-10 h-10 rounded-full"
              viewBox="0 0 32 32"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <title>PHP</title>
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <circle cx="16" cy="16" r="14" fill="#8892BF"></circle>{" "}
                <path
                  d="M14.4392 10H16.1192L15.6444 12.5242H17.154C17.9819 12.5419 18.5986 12.7269 19.0045 13.0793C19.4184 13.4316 19.5402 14.1014 19.3698 15.0881L18.5541 19.4889H16.8497L17.6288 15.2863C17.7099 14.8457 17.6856 14.533 17.5558 14.348C17.426 14.163 17.146 14.0705 16.7158 14.0705L15.3644 14.0573L14.3661 19.4889H12.6861L14.4392 10Z"
                  fill="white"
                ></path>{" "}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M6.74092 12.5243H10.0036C10.9612 12.533 11.6552 12.8327 12.0854 13.4229C12.5156 14.0132 12.6576 14.8193 12.5115 15.8414C12.4548 16.3085 12.3289 16.7665 12.1341 17.2159C11.9474 17.6652 11.6878 18.0704 11.355 18.4317C10.9491 18.8898 10.5149 19.1805 10.0523 19.304C9.58969 19.4274 9.11076 19.489 8.61575 19.489H7.15484L6.69222 22H5L6.74092 12.5243ZM7.43485 17.9956L8.16287 14.0441H8.40879C8.49815 14.0441 8.5914 14.0396 8.6888 14.0309C9.33817 14.0221 9.87774 14.0882 10.308 14.2291C10.7462 14.37 10.8923 14.9031 10.7462 15.8282C10.5678 16.9296 10.2186 17.5727 9.69926 17.7577C9.1799 17.934 8.53053 18.0176 7.75138 18.0088H7.58094C7.53224 18.0088 7.48355 18.0043 7.43485 17.9956Z"
                  fill="white"
                ></path>{" "}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M24.4365 12.5243H21.1738L19.4329 22H21.1251L21.5878 19.489H23.0487C23.5437 19.489 24.0226 19.4274 24.4852 19.304C24.9479 19.1805 25.382 18.8898 25.7879 18.4317C26.1207 18.0704 26.3803 17.6652 26.567 17.2159C26.7618 16.7665 26.8877 16.3085 26.9444 15.8414C27.0905 14.8193 26.9486 14.0132 26.5183 13.4229C26.0881 12.8327 25.3942 12.533 24.4365 12.5243ZM22.5958 14.0441L21.8678 17.9956C21.9165 18.0043 21.9652 18.0088 22.0139 18.0088H22.1843C22.9635 18.0176 23.6128 17.934 24.1322 17.7577C24.6515 17.5727 25.0007 16.9296 25.1792 15.8282C25.3253 14.9031 25.1792 14.37 24.7409 14.2291C24.3107 14.0882 23.7711 14.0221 23.1217 14.0309C23.0243 14.0396 22.9311 14.0441 22.8417 14.0441H22.5958Z"
                  fill="white"
                ></path>{" "}
              </g>
            </svg> */}
          </div>
          <p className="uppercase py-4 gext-gray-300">
            {start} - {end}
          </p>
          <ul className="list-disc space-y-4 ml-5 stext-lg">
            {summary && summary.map((item, i) => <li key={i}>{item}</li>)}
            {/* <li>Summary Points Summary Points Summary Points</li>
            <li>Summary Points Summary Points Summary Points</li>
            <li>Summary Points Summary Points Summary Points</li>
            <li>Summary Points Summary Points Summary Points</li>
            <li>Summary Points Summary Points Summary Points</li> */}
          </ul>
        </div>
      </article>
    </motion.div>
  );
}
